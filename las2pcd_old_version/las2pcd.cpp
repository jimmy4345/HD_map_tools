...
int main (int argc, char *argv[])
{
	...
	for (unsigned int i = 0;i < files.size();i++){
		...
		cerr << "PointCloud detect: " << nbPoints << " data points in " << lasfile_path << endl;
		...
		while(reader.ReadNextPoint())
		{
			...
			// get XYZ information
			point.x = reader.GetPoint().GetX();
			point.y = reader.GetPoint().GetY();
			point.z = reader.GetPoint().GetZ();

			// get Intensity information
			point.intensity = reader.GetPoint().GetIntensity();
		...
		writer.write<pcl::PointXYZI>(output_dir + "/" + files[i].replace(files[i].find("las"),3,"pcd"), *cloud, false);

		cerr << "Saved " << cloud->width * cloud->height << output_dir + "/" + files[i] << endl;
		cout << endl;
